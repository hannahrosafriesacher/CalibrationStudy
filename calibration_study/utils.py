...
def get_predictions(net, X_val, X_test, X_train):
    '''
    Obtaines classification scores and hidden layer of Train, Validation and Test Set.
    '''
    pred_val, pred_val_hidden = net(X_val, return_hidden=1)
    pred_test, pred_test_hidden = net(X_test, return_hidden=1)
    pred_train, pred_train_hidden = net(X_train, return_hidden=1)
    return {
        'val': pred_val.detach().cpu(),
        'test': pred_test.detach().cpu(),
        'train': pred_train.detach().cpu(),
        'val_hidden': pred_val_hidden.detach().cpu(),
        'test_hidden': pred_test_hidden.detach().cpu(),
        'train_hidden': pred_train_hidden.detach().cpu()
    }
